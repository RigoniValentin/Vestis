...
ProductSchema.pre("save", function (next) {
    console.log(`🔧 Pre-save middleware ejecutándose para producto: ${this.name}`);
...
ProductSchema.pre("save", function (next) {
    try {
        ...
        if (this.images && this.images.length > 4) {
            console.log(`❌ Demasiadas imágenes: ${this.images.length}, máximo permitido: 4`);
            return next(new Error("No se pueden tener más de 4 imágenes por producto"));
        }
        ...
        console.log(`✅ Validación de imágenes exitosa: ${this.images?.length || 0} imágenes`);
        next();
    }
    catch (error) {
