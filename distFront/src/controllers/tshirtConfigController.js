...
exports.getTshirtConfigStats = exports.deleteTshirtConfig = exports.toggleTshirtConfigActive = exports.updateTshirtConfigStock = exports.updateTshirtConfig = exports.createTshirtConfig = exports.findMatchingConfig = exports.getTshirtConfigById = exports.getTshirtConfigs = void 0;
...
// @access  Public
const getTshirtConfigs = async (req, res) => {
    try {
        const { tshirtType, design, color, isActive } = req.query;
        let query = {};
        if (tshirtType)
            query.tshirtType = tshirtType;
        if (design)
            query.design = design;
        if (color)
        ...
        const configs = await TshirtConfig_1.default.find(query)
...
exports.getTshirtConfigs = getTshirtConfigs;
// @desc    Obtener una configuración por ID
// @route   GET /api/v1/tshirt-configs/:id
// @access  Public
const getTshirtConfigById = async (req, res) => {
    try {
        const config = await TshirtConfig_1.default.findById(req.params.id)
            .populate("tshirtType")
            .populate("design");
        if (!config) {
            res.status(404).json({
...
const findMatchingConfig = async (req, res) => {
    try {
        ...
        if (size) {
            const stockEntry = config.stock.find((s) => s.size === size.toUpperCase());
...
exports.findMatchingConfig = findMatchingConfig;
// @desc    Crear una nueva configuración
// @route   POST /api/v1/tshirt-configs
// @access  Admin
const createTshirtConfig = async (req, res) => {
    try {
        ...
        const existing = await TshirtConfig_1.default.findOne({
            tshirtType,
            design,
            color: color?.toLowerCase(),
        });
        ...
        const config = await TshirtConfig_1.default.create({
            tshirtType,
            design,
            color: color?.toLowerCase(),
            sizes: parsedSizes,
        ...
        const populatedConfig = await TshirtConfig_1.default.findById(config._id)
            .populate("tshirtType", "description productType")
            .populate("design", "name description imageUrl year tags");
        res.status(201).json({
            success: true,
...
exports.createTshirtConfig = createTshirtConfig;
// @desc    Actualizar una configuración
// @route   PUT /api/v1/tshirt-configs/:id
// @access  Admin
const updateTshirtConfig = async (req, res) => {
    try {
        ...
        if (productImage && req.file) {
            const existingConfig = await TshirtConfig_1.default.findById(req.params.id);
            if (existingConfig && existingConfig.productImage) {
                (0, uploadTshirtConfig_1.deleteConfigImage)(existingConfig.productImage);
            }
        }
        // Parsear sizes y stock si vienen como strings (desde FormData)
        ...
        const updateData = {};
        if (tshirtType)
            updateData.tshirtType = tshirtType;
        if (design)
        ...
        const config = await TshirtConfig_1.default.findByIdAndUpdate(req.params.id, updateData, { new: true, runValidators: true })
            .populate("tshirtType", "description productType")
            .populate("design", "name description imageUrl year tags");
        if (!config) {
            res.status(404).json({
...
exports.updateTshirtConfig = updateTshirtConfig;
// @desc    Actualizar stock de una configuración
// @route   PUT /api/v1/tshirt-configs/:id/stock
// @access  Admin
const updateTshirtConfigStock = async (req, res) => {
    try {
        ...
        const config = await TshirtConfig_1.default.findByIdAndUpdate(req.params.id, { stock }, { new: true, runValidators: true })
            .populate("tshirtType", "description productType")
            .populate("design", "name description imageUrl year tags");
        if (!config) {
            res.status(404).json({
...
exports.updateTshirtConfigStock = updateTshirtConfigStock;
// @desc    Activar/Desactivar una configuración
// @route   PATCH /api/v1/tshirt-configs/:id/toggle-active
// @access  Admin
const toggleTshirtConfigActive = async (req, res) => {
    try {
        const config = await TshirtConfig_1.default.findById(req.params.id);
        if (!config) {
            res.status(404).json({
...
exports.toggleTshirtConfigActive = toggleTshirtConfigActive;
// @desc    Eliminar una configuración
// @route   DELETE /api/v1/tshirt-configs/:id
// @access  Admin
const deleteTshirtConfig = async (req, res) => {
    try {
        const config = await TshirtConfig_1.default.findByIdAndDelete(req.params.id);
        if (!config) {
            res.status(404).json({
                success: false,
                message: "Configuración no encontrada",
...
exports.deleteTshirtConfig = deleteTshirtConfig;
// @desc    Obtener estadísticas de configuraciones
// @route   GET /api/v1/tshirt-configs/stats/overview
// @access  Admin
const getTshirtConfigStats = async (req, res) => {
    try {
        const totalConfigs = await TshirtConfig_1.default.countDocuments();
        const activeConfigs = await TshirtConfig_1.default.countDocuments({ isActive: true });
        const inactiveConfigs = totalConfigs - activeConfigs;
        // Configs con stock bajo (algún talle con < 5 unidades)
        const configs = await TshirtConfig_1.default.find({ isActive: true });
        let lowStockCount = 0;
        let outOfStockCount = 0;
        configs.forEach((config) => {
            const hasLowStock = config.stock.some((s) => s.quantity > 0 && s.quantity < 5);
            const allOutOfStock = config.stock.length === 0 ||
...
exports.getTshirtConfigStats = getTshirtConfigStats;
